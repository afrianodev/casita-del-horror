...
export default function Previews({ onMovieSelect }) {
  const [randomMovie, setRandomMovie] = useState(null);

  useEffect(() => {
    // Set a random movie only once when the component mounts
  ...
  if (!randomMovie) return null; // Prevent rendering until a movie is selected
    ...
    <div className="relative h-[450px] md:h-[360px] lg:h-[400px] w-[100%] md:w-[100%] overflow-hidden mt-2">
      ...
      <div className="relative z-10 flex justify-center items-center w-full h-[450px] md:h-[360px] lg:h-[400px] gap-4 ml-[5%]">
        ...
        <div className="flex flex-col gap-1 md:gap-4 w-[350px]">
          ...
          <div className="flex gap-2">
            <div
              className="text-white bg-black w-[120px] text-center py-2 rounded-md cursor-pointer select-none"
              onClick={() => handleMovieClick(randomMovie)}
            >
              Ver Película
            </div>
          </div>
